...
### 4.4 Byte payload API и base64 фиксированного размера

Реализация предоставляет DPUID bit stream как переиспользуемый byte payload:
...
Нулевой размер выбирает layout переменной длины из §4.3. Положительный размер
задаёт фиксированный payload: Variant 1 использует явный count шириной
`OUTPUT_POW - 1`, Variants 2/3 используют под count все биты после `D`, как в
UUID modes. Незаписанные биты равны нулю, длина результата — `ceil(bits / 8)`.

UUIDv8 и raw UUID переиспользуют codec с 122 и 128 data bits соответственно.
Base64 wrappers кодируют полученные bytes по RFC 4648. Вариант без размера
передаёт ноль, вариант с размером — заданный bit limit.

Для произвольного положительного bit limit cap ширин полей определяется
минимальным контейнером со степенью двойки, покрывающим limit. Поэтому 122 и 128
data bits используют cap 7.
...
|---|---|---|
| 1.1.0 | 2026-06-19 | UUID modes вычисляют `OUTPUT_POW` из типа элемента с ограничением `log2(OUTPUT_BITS)`; обновлены §§3–5, 8–10, 14–15 |
